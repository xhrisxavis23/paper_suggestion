...
| `--deep` | off | v0.4 I-1: pull PDFs for the top `--deep-k` matched papers, extract intro/method/limitations sections, and feed into Skeptic + Proposer. Trend / Gap stay metadata-only by design. PDFs are cached at `metadb/.pdfs/` (gitignored, immutable per paper id) AND hardlinked into `reports/<report-stem>/<short-id>.pdf` (also gitignored) with a `manifest.json` describing what's there — disk impact is hardlinks only. Adds ~10-15 sec wall and ~$0.01-$0.03 token cost. |
