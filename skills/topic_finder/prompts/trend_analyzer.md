...
3. 각 클러스터마다:
   ...
   - `paper_ids`: 속한 논문의 id 리스트 (**최대 30개**; 더 많은 경우 가장 핵심적인 30편으로 제한해 토큰 예산을 보호한다)
   - `period_count`: 입력 논문들의 date 범위를 **4등분**한 각 구간의 빈도 (오래된 → 최근 순, 길이 4 정수 배열). 윈도우 길이와 무관하게 데이터의 실제 min~max 날짜 범위를 4등분.
...
## 금기
- 추측 기반 빈도 (`period_count`는 입력 데이터의 date 필드만 보고 산출)
