...
- **`--deep` mode (v0.4 I-1)**: pulls PDFs for the top `--deep-k` (default 10) matched papers, extracts intro / method / limitations via pypdf + heading heuristics in `skills/topic_finder/scripts/pdf_extract.py`, caches under `metadb/.pdfs/` (gitignored, paper-id keyed, immutable), and prepends the resulting deep-context block to the Skeptic + Proposer user messages only. Trend / Gap stay metadata-only by design. Failure modes are graceful: any single paper that 4xx/5xx's or that pypdf can't parse is silently dropped from the context.
