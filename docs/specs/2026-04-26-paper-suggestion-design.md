...
metadb/
├── daily/
│   ├── 2026-04-26.md             # 일별 다이제스트
│   └── ... (60일분 보관)
├── 2602_rolling.jsonl            # 월별 인덱스 (YYMM = 26년 02월)
├── 2603_rolling.jsonl
├── 2604_rolling.jsonl            # 한 줄 = 한 논문, published_date의 YYMM 기준 분할
└── stats.json                    # 마지막 실행 시각, 소스별 건수, 실패 목록
```

> Note: rolling DB는 `<YYMM>_rolling.jsonl` 파일들로 분할 저장됩니다 (개별 파일 ≤ ~30 MB 유지, GitHub 50 MB 권장치 안). RollingDB는 디렉터리를 통째로 받아 모든 월 파일을 합쳐 읽습니다.
